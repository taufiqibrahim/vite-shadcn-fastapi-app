...
async def test_update_user_success(client, test_account_authorized_headers):
    ...
    response = client.patch(
        "/api/v1/users/me", headers=test_account_authorized_headers, json=update_data
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
...
async def test_create_user_profile_success(
    client, test_account_authorized_headers, test_account
):
...
async def test_get_user_profile_success(client, test_account_authorized_headers):
    ...
    response = client.get(
        "/api/v1/users/me/profile", headers=test_account_authorized_headers
    )
    assert response.status_code == status.HTTP_200_OK
    data = response.json()
