...
from src.auth import models  # Import the Account model
from src.auth.services import (
    get_current_active_account,
    get_current_account_by_api_key,
    get_current_account_or_400,
...
@router.post(
    "/", response_model=schemas.Account, status_code=status.HTTP_201_CREATED
)
async def create_user(
    ...
    if account.account_type == "machine":
        raise HTTPException(status_code=400, detail="Cannot create machine account via this endpoint")
    created_account = services.create_account(db=db, user=account)
    profile = schemas.UserProfileCreate(full_name=account.full_name)
